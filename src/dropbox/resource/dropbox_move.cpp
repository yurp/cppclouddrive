...
move::move(ccd::http::transport_func http_transport, std::string from_path, std::string to_path)
...
ccd::http::request move::build_request()
{
    ccd::http::request e;
    e.method = "POST";
    e.host = "https://api.dropboxapi.com";
    e.path = "/2/files/move_v2";
...
move& move::set_allow_shared_folder(bool x)
{
    m_json[allow_shared_folder_field] = x;
    return *this;
}

move& move::set_autorename(bool x)
{
    m_json[autorename_field] = x;
    return *this;
}

move& move::set_allow_ownership_transfer(bool x)
{
    m_json[allow_ownership_transfer_field] = x;
    return *this;
}

boost::future<model::metadata> move::exec()
{
